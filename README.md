![Mod loader: Fabric](https://img.shields.io/badge/modloader-Fabric-1976d2?style=flat-square&logo=data:image/png;base64,)
![requires: FabricAPI](https://img.shields.io/badge/requires-Fabric_API-important)

# Nether Boats

Fabric mod which adds Warped and Crimson boats, which sink in lava instead of burning. Requires Fabric API (I was too lazy to make it without it...)

## Compile

First, run these commands to clone the repository:
```shell script
git clone https://github.com/samipourquoi/nether-boats.git
cd nether-boats
```

Then run one of these commands to compile the mod:
```shell script
# Windows
gradlew build

# macOS or Linux
./gradlew build
```

The compiled jar will be in `./build/libs/` and will be called `nether-boats.<version>.jar`.

## Credits

Suggestion by @Epiphany#8628. Coded by samipourquoi.
